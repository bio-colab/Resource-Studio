...
| Testing | اختبارات core/CLI/QA/MCP/golden، PE corpus matrix متعدد المعمارية والprofiles، bounded وstructure-aware fuzzing، crash consistency، Win32 oracles، وبوابة بناء WPF في CI |
...
| `mcp==2.0.0` و`httpx>=0.27,<1` | خادم MCP المحلي (stdio) وطبقة Streamable HTTP الاختيارية |
...
بناء WPF وتشغيل اختبارات core/QA على Windows مغطاة في مهمة Windows داخل `.github/workflows/ci.yml`؛ وتشمل الاختبارات المربوطة أوراكل Windows الحقيقية (`test_windows_resource_oracle` و`test_win32_update_resource_oracle`) وتعمل فعليًا عند توفر Win32.
...
وجولة أداء معمارية لاحقة أزالت ثقل الإقلاع: `core/__init__.py` أصبح واجهة كسولة (PEP 562) فلا يسحب `lief` إلا عند حاجة فعلية لفك PE (`cli --help` من 472ms إلى 53ms)، وكل استدعاءات CLI من WPF في وضع source تمر عبر [`tools/wpf_cli_host.py`](tools/wpf_cli_host.py) الدائم عديم الحالة بنفس بروتوكول read host مع fallback تلقائي إلى spawn؛ تسلسل 4 عمليات محرر انخفض من ~1743ms إلى ~456ms والنداء الدافئ من ~393ms إلى ~6ms. التفاصيل والقياسات في [`CODE-REVIEW.md`](CODE-REVIEW.md).

جولة الأيض اللاحقة أزالت الهدر داخل العملية نفسها: أجهزة التقارير للقراءة فقط تشترك الآن في تفكيك LIEF واحد لكل ملف عبر [`core/parse_cache.py`](core/parse_cache.py) (محلي-الخيط، محدود بـ4 مداخيل/256MB، keyed بـpath+size+mtime_ns) فانخفض `inspect` من 8 تفكيكات إلى 1 والكتابة من 11 إلى 5، بينما بقيت التفكيكات المتحولة (writer، strip، oracle) خاصة. كما أُعيدت كتابة `preservation._diff_ranges` بمقارنة مجزأة 64KB فأصبحت كلفة تتبع المنطقة المتغيرة بدل كامل الملف (ملف 64MB: 15.7s → 21ms، ×742)، مع إصلاح عمى طرفي كان يُسقط أي نطاق تغيّر يصل إلى نهاية الملف من خريطة الحفظ (البايتات الملحقة/المقتطعة عند EOF لم تكن تُبلّغ أبدًا) مع اختبار انحدار تفاضلي. كتابة 64MB كاملة تنتهي الآن في ~4.8s بدل ~17s بالمكونات المقيسة سابقًا.
...
- [Core architecture reference](docs/CORE-ARCHITECTURE.md)
- [Roadmap](docs/ROADMAP.md)
- [Batch manifest format](docs/BATCH-MANIFEST.md)
- [Hex templates and triage](docs/HEX-TEMPLATES-AND-TRIAGE.md)

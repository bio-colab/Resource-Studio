...
اسم الملف المضغوط هو `ResourceStudio-0.1.0-win-x64-installer.zip`. يحتوي ZIP على `ResourceStudio-Setup-0.1.0-win-x64.exe`.
...
```powershell
Get-FileHash .\ResourceStudio-0.1.0-win-x64-installer.zip -Algorithm SHA256
```

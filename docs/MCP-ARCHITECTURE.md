...
يوفر الخادم ثلاثة موارد read-only قابلة للاكتشاف: `resource://tools/metadata` لوصف أثر كل أداة وشروطها ومتطلبات التأكيد والتدقيق، و`resource://session/state` لملخص حالة الجلسة، و`resource://session/events` لسجل محدود لأحداث التشغيل. هذه الموارد لا تمنح صلاحية جديدة ولا تستبدل `operation_audit` أو Evidence Ledger، ولا تحول الأحداث إلى verdict جنائي. الغرض هو جعل حالة MCP وآثار الأدوات واضحة للعميل والمراجع قبل وبعد دورة `plan -> confirm -> apply -> verify`.

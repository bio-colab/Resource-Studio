...
أضيف `tests/qa/test_p4_wpf_session_contract.py` ليثبت وجود request generation وstale guards ومرجعية cancellation وprocess ownership (أُزيل لاحقًا ضمن تنظيف الفحوص الزائدة لأنه عقد نصي دون تنفيذ كود؛ القرار والتبرير في `CODE-REVIEW.md`). كما بقي اختبار `wpf_read_host` وCLI ناجحًا. التحقق النهائي من بناء WPF يتم عبر Windows CI.

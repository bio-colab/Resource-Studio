    ...
    def _plan(
        ...
        with tempfile.TemporaryDirectory(prefix="resource-studio-plan-") as temporary:
            ...
            return {
                ...
                "beforeSha256": sha256_file(input_path),
                "proposedSha256": sha256_file(output),
                "isPE": report.is_pe,
    ...
    def _write(
        ...
        try:
            ...
            try:
                ...
                # Best-effort diagnostics: the failure path must not crash on
                # top of itself when diagnostics cannot be written.
                pass

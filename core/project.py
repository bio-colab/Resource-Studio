...
class Project:
    ...
    @contextmanager
    def locked(self):
        """Exclusive-access context over acquire_lock/release_lock with
        stale-lock recovery (TODO GAP-01); reserved for session and MCP
        project concurrency."""
        self.acquire_lock()
        try:
            yield self
    ...
    def open_pe(cls, source_path: Path, project_dir: Path) -> Project:
        ...
        try:
            ...
            binary = shared_parse(source_path)
    ...
    def apply_typed_resource(
        ...
        *,
        ...
        """Project-mode typed write: wraps LiefPEWriter with workspace isolation,
        backup, re-index, and audit. Called by tests today; the designed consumers
        are the pending typed visual editors (TODO EDIT-05..08)."""
        if self.workspace_path is None or not self.workspace_path.is_file():
            raise ValueError("project has no isolated PE workspace")
        from .pe_writer import LiefPEWriter
    ...
    def apply_version_info(
        ...
        *,
        ...
        """Project-mode VERSION write (see apply_typed_resource)."""
    ...
    def apply_dialog(
        ...
        *,
        ...
        """Project-mode DIALOG write (see apply_typed_resource)."""
    ...
    def apply_res_record(self, record: Any, *, add: bool = False) -> Path:
        """Project-mode .res record write (see apply_typed_resource)."""
        if self.workspace_path is None or not self.workspace_path.is_file():
            raise ValueError("project has no isolated PE workspace")
        from .pe_writer import LiefPEWriter
    ...
    def apply_manifest(self, manifest_xml: str) -> Path:
        """Project-mode manifest replacement (see apply_typed_resource)."""
        if self.workspace_path is None or not self.workspace_path.is_file():
            raise ValueError("project has no isolated PE workspace")
        from .pe_writer import LiefPEWriter
    ...
    def restore_snapshot(self, snapshot: str | Path, *, backup_current: bool = True) -> Path:
        """Crash-recovery half of snapshot(): restore the JSON+workspace pair
        with backup and resource verification (TODO CORE-04)."""
        if self.workspace_path is None or not self.workspace_path.is_file():
            raise ValueError("project has no isolated PE workspace")

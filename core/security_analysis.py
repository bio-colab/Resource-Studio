...
def analyze_security(path: Path) -> dict[str, Any]:
    ...
    try:
        ...
        base["target"].update({"sha256": hashlib.sha256(data).hexdigest(), "size": len(data)})
    ...
    try:
        ...
        base["staticIndicators"] = _static_indicators(inspector, deep)
...
def _static_indicators(inspector: dict[str, Any], deep: dict[str, Any]) -> list[dict[str, Any]]:

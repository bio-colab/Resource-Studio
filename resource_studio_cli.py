...
def _diff_payload(left_path: Path, right_path: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "fileChanged": sha256_file(left_path) != sha256_file(right_path),
        "changes": changes,
        "tree": tree,
    }
...
def command_build(args: argparse.Namespace) -> int:
    ...
    _print({"output": str(output), "sha256": sha256_file(output)}, args.json)
    return 0
...
def command_security(args: argparse.Namespace) -> int:
    ...
    target_sha256 = sha256_file(args.input)
...
def parser() -> argparse.ArgumentParser:
    ...
    # Imported here (not at parser() top) so --help and the other 34
    # subcommands never load core.reports during startup.

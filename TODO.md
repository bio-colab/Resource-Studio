...
**MCP:** طبقة MCP المحلية منفذة ومختبرة ومربوطة ببوابة CI (stdio وHTTP وplugins وintegrations وpackages وobservability)؛ أي توسعة جديدة تتبع نسخ العقد في [`docs/MCP-CONTRACT.md`](docs/MCP-CONTRACT.md).  
**السلامة:** لا تُكتب ملفات الاختبار أو مخرجات المشروع إلى مجلدات التثبيت المحمية.
**بيئة الاختبار:** نسخة عمل معزولة على Windows.

> **ملاحظة أرشيفية:** صفوف السجل التاريخي أدناه تذكر بعض الملفات التي أُزيلت لاحقًا ضمن تنظيف الفحوص الزائدة (`tests/windows/`، عقود WPF النصية من `tests/qa/`، و17 وثيقة من `docs/`)؛ القرارات والتبريرات موثقة في [`CODE-REVIEW.md`](CODE-REVIEW.md).

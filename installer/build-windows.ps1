...
$env:PYTHONPATH = $Root
Invoke-Checked $py.Source @('-3.12', '-m', 'PyInstaller', '--version')
Invoke-Checked $py.Source @(
    '-3.12', '-m', 'PyInstaller',
...
& $py.Source '-3.12' (Join-Path $Root 'installer\make-assets.py')
...
if (-not $SkipInstaller) {
    ...
    Invoke-Checked $iscc.FullName @('/Qp', $iss, "/DAppVersion=$Version", "/DSourceDir=$Stage", "/DOutputDir=$Output")
}

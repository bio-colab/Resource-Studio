...
# Script-mode launch (py.exe -3.12 tools/wpf_read_host.py) puts tools/ on
# sys.path[0] and NOT the repo root, so 'import resource_studio_cli' fails.
# Self-heal before importing project modules; harmless when PYTHONPATH is set.
_ROOT = Path(__file__).resolve().parent.parent
if str(_ROOT) not in sys.path:
    sys.path.insert(0, str(_ROOT))

import resource_studio_cli as cli  # noqa: E402
from core.resource_reader import ResourceReader  # noqa: E402


def dispatch_cli(argv: list[str]) -> tuple[int, str]:
    """Run one CLI invocation in-process; shared with tools/wpf_cli_host.py."""
    stdout = io.StringIO()
    stderr = io.StringIO()
    try:
...
class ReadSession:
    ...
    _dispatch_cli = staticmethod(dispatch_cli)
